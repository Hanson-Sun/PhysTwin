...
def main():
    ...
    parser.add_argument('--eval-max-clips',   type=int, default=None,
    ...
    if not args.skip_train:
        ...
        if args.resume:
            ...
            if checkpoint.exists():
                ...
                print(f"   ⚠ --resume set but no checkpoint found at {checkpoint}, starting fresh")
    ...
    if not args.skip_eval:
        if not checkpoint.exists():
            ...
            if args.eval_max_clips:
                ...
                print(f"   Evaluating on {len(clips)} clips (--eval-max-clips={args.eval_max_clips})")
            else:
            ...
            for clip in clips:
                ...
                cmd = [
                    ...
                    '--depth-input',      str(clip['depth']),
                    '--rgb-input',        str(clip['rgb']),
                    '--vda-input',        str(clip['vda']),
                    '--output-dir',       str(clip_eval_dir),
                    '--motion-threshold', str(args.motion_threshold),
                    '--depth-threshold',  str(args.depth_threshold),
                    '--vda-weight',       str(args.vda_weight),
                    '--save-smoothed',
                ]
                if args.visualize:
                ...
                if not run_command(cmd, f'EVALUATION — {clip["clip_id"]}'):
                    print(f"   ⚠ Evaluation failed for {clip['clip_id']}, continuing...")
                    all_ok = False
