...
def save_adjusted_calibration(
    output_root: Path,
    camera_ids: List[int],
    adjusted_intrinsics: Optional[np.ndarray],
    adjusted_extrinsics_c2w: Optional[np.ndarray],
    original_intrinsics: List[np.ndarray],
    original_extrinsics_w2c: List[np.ndarray]
):
    """
    Save adjusted calibration data returned by DA3 model.
    
    The adjusted calibration reflects any changes made by the model
    (e.g., to match the depth output resolution).
    
    Args:
        output_root: Output directory
        camera_ids: List of camera IDs
        adjusted_intrinsics: Adjusted intrinsics (N, 3, 3) or None
        adjusted_extrinsics_c2w: Adjusted extrinsics as c2w (N, 4, 4) or None
        original_intrinsics: Original intrinsics for fallback
        original_extrinsics_w2c: Original extrinsics (w2c) for fallback
    """
    # Use adjusted if available, otherwise use original
    if adjusted_intrinsics is not None:
        intrinsics_to_save = adjusted_intrinsics
    else:
        intrinsics_to_save = np.stack(original_intrinsics, axis=0)
    
    if adjusted_extrinsics_c2w is not None:
        c2w_to_save = adjusted_extrinsics_c2w
    else:
        # Convert original w2c to c2w
        c2w_to_save = np.array([np.linalg.inv(w2c) for w2c in original_extrinsics_w2c])
    
    # Save as calibrate_adjusted.pkl (list of c2w matrices)
    calib_path = output_root / "calibrate_adjusted.pkl"
    calib_list = [c2w_to_save[i] for i in range(len(camera_ids))]
    
    with open(calib_path, "wb") as f:
        pickle.dump(calib_list, f)
    
    # Save as metadata_adjusted.json (intrinsics)
    metadata_path = output_root / "metadata_adjusted.json"
    metadata = {
        "intrinsics": [intrinsics_to_save[i].tolist() for i in range(len(camera_ids))]
    }
    ...
    # Print summary
    print("\n" + "=" * 80)
    print("Adjusted Calibration Saved")
    print("=" * 80)
    print(f"  calibrate_adjusted.pkl → {calib_path}")
    print(f"  metadata_adjusted.json → {metadata_path}")
    
    if adjusted_intrinsics is not None:
        print("\n  Adjusted intrinsics (camera 0):")
        print(f"    fx={intrinsics_to_save[0][0,0]:.2f}, fy={intrinsics_to_save[0][1,1]:.2f}")
        print(f"    cx={intrinsics_to_save[0][0,2]:.2f}, cy={intrinsics_to_save[0][1,2]:.2f}")
    else:
        print("\n  Note: Using original intrinsics (model did not return adjusted values)")
    
    if adjusted_extrinsics_c2w is not None:
        print("  Adjusted extrinsics: ✓ (from model)")
    else:
        print("  Note: Using original extrinsics (model did not return adjusted values)")
...
def process_chunk(
    ...
    device: torch.device
) -> Tuple[Dict[int, List[np.ndarray]], Optional[np.ndarray], Optional[np.ndarray]]:
    """
    ...
    Returns:
        ...
        - Adjusted intrinsics (N_cams, 3, 3) or None
        - Adjusted extrinsics (N_cams, 4, 4) c2w or None
    """
    ...
    # Extract adjusted calibration if available
    adjusted_intrinsics = None
    adjusted_extrinsics_c2w = None
    
    if hasattr(prediction, 'intrinsics') and prediction.intrinsics is not None:
        adjusted_intrinsics = np.array(prediction.intrinsics)
    
    if hasattr(prediction, 'extrinsics') and prediction.extrinsics is not None:
        adjusted_extrinsics = np.array(prediction.extrinsics)
        # DA3 returns w2c in (N, 3, 4) or (N, 4, 4) - convert to (N, 4, 4) c2w
        adjusted_c2w = []
        for i in range(len(adjusted_extrinsics)):
            ext = adjusted_extrinsics[i]
            # Convert to 4x4 w2c if needed
            if ext.shape == (3, 4):
                w2c_4x4 = np.eye(4)
                w2c_4x4[:3, :4] = ext
            else:
            ...
            adjusted_c2w.append(c2w)
        adjusted_extrinsics_c2w = np.array(adjusted_c2w)
    ...
    # Extract per-camera calibration for the first frame
    if adjusted_intrinsics is not None:
        # Take calibration from first frame (0 to num_cams-1)
        adjusted_intrinsics = adjusted_intrinsics[:num_cams]
    
    if adjusted_extrinsics_c2w is not None:
        # Take calibration from first frame
        adjusted_extrinsics_c2w = adjusted_extrinsics_c2w[:num_cams]
    
    return chunk_depths, adjusted_intrinsics, adjusted_extrinsics_c2w
...
def process_multi_camera_streaming(
    ...
    # Track if we've saved calibration yet
    ...
    try:
        ...
        while start_idx < num_frames:
            ...
            # Process chunk
            chunk_depths, adjusted_intrinsics, adjusted_extrinsics_c2w = process_chunk(
                model=model,
            ...
            # Save adjusted calibration from first chunk
            if not calibration_saved and (adjusted_intrinsics is not None or adjusted_extrinsics_c2w is not None):
                save_adjusted_calibration(
                    output_root=output_root,
                    camera_ids=camera_ids,
                    adjusted_intrinsics=adjusted_intrinsics,
                    adjusted_extrinsics_c2w=adjusted_extrinsics_c2w,
                    original_intrinsics=[cfg.intrinsics for cfg in camera_configs],
                    original_extrinsics_w2c=[cfg.extrinsics for cfg in camera_configs]
                )
                calibration_saved = True
                if args.verbose:
                    tqdm.write("  ✓ Saved adjusted calibration")
