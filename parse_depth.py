...
def project_points_to_camera(points_world, K, w2c, h, w):
    """Project 3D world points to camera and create depth map"""
    ...
    x_valid = x[valid].astype(np.int32)
    y_valid = y[valid].astype(np.int32)
    z_valid = z[valid]
    
    # Fill depth map (keep closest depth for each pixel)
    for xi, yi, zi in zip(x_valid, y_valid, z_valid):
        if depth_map[yi, xi] == 0 or zi < depth_map[yi, xi]:
            depth_map[yi, xi] = zi
...
def main():
    ...
    for frame_idx, frame_name in enumerate(tqdm(common_frames, desc="Processing")):
        ...
        # Compute alignment transform using camera poses
        ...
        for i, k in enumerate(calib_keys):
            # Get points from this camera's view only
            pts_cam = pts3d[i].detach().cpu().numpy()
            h, w = pts_cam.shape[:2]
            pts_flat = pts_cam.reshape(-1, 3)
            
            # Transform: camera space -> predicted world -> aligned GT world
            pred_c2w = pred_poses[i]
            pts_pred_world = (pts_flat @ pred_c2w[:3, :3].T) + pred_c2w[:3, 3]
            pts_gt_world = scale * (pts_pred_world @ R_align.T) + t_align
            ...
            # Project points back to this camera's GT viewpoint
            depth_map = project_points_to_camera(pts_gt_world, scaled_k, gt_w2c, h, w)
