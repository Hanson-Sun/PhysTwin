...
def calculate_z_shift(points: np.ndarray, method: str = "inlier", 
    ...
    """Calculate z-shift to ensure all points have negative z values (below camera plane)."""
    z = points[..., 2]
    
    if method == "inlier":
        mean, std = np.mean(z), np.std(z)
        z_inliers = z[np.abs(z - mean) <= std_threshold * std]
        max_z = np.max(z_inliers) if len(z_inliers) > 0 else np.max(z)
    elif method == "max":
        max_z = np.max(z)
    else:
        raise ValueError(f"Unknown method: {method}")
    
    # Return negative shift to pull all points below zero
    return -(max_z + 0.001) if max_z >= 0 else 0.0
...
def calibrate_mesh(mesh_path: str, shift_amount: float) -> bool:
    ...
    try:
        ...
        if isinstance(mesh, trimesh.Scene):
            # For scenes, merge all geometries into a single mesh
            geometries = []
            ...
            if len(geometries) > 1:
                merged_mesh = trimesh.util.concatenate(geometries)
...
if __name__ == "__main__":
    ...
    if not info.get("shifted"):
        ...
        # Verify camera extrinsics shifted
        cam_z_before = c2ws_old[0][2, 3]
        cam_z_after = c2ws[0][2, 3]
        print(f"  Camera z-position: {cam_z_before:.6f} → {cam_z_after:.6f}")
        
        # Verify all points are below zero
        if "z_after" in info and info["z_after"][1] < 0:
            print(f"  ✓ All points now below z=0: [{info['z_after'][0]:.6f}, {info['z_after'][1]:.6f}]")
        else:
            print(f"  ⚠ WARNING: Some points still above z=0!")
